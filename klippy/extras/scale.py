...
class Scale():
    def __init__(self, config):
        ...
        self.timer = self.reactor.register_timer(
            self._measure_n_check_timer, self.reactor.monotonic() + 5.)
    ...
    def get_weight(self, eventtime):
        values = [s.get_weight(average=self.is_printing(eventtime))
                  for s in self.sensors]
...
class ScaleMaster():
    def __init__(self, config):
        ...
        self.devices = self.printer.lookup_objects('scale')
